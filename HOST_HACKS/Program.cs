    ...
    class Program
    {
        ...
        private static void OnTimedEvent(Object source, System.Timers.ElapsedEventArgs e)
        {
            if (frm.textBox1.Text == "MAP")
            {
                ...
                {
                    lines2[i] = Pointer("dota2", Convert.ToInt32(lines[i]), new int[] { 0, 0x71C}, true, 0).Value.ToString("X");
                    lines3[i] = Pointer("dota2", Convert.ToInt32(lines[i]), new int[] { 0, 0x720 }, true, 0).Value.ToString("X");
