...
namespace HOST_HACKS
{
    class Program
    {
        ...
        static string OffsetGoldR = "server.dll+1D96010";
        static string OffsetGoldD = "server.dll+1D96018";
	static string OffsetPlayer = "server.dll+01DA2B30";
        ...
        private static void OnTimedEvent(Object source, System.Timers.ElapsedEventArgs e)
        {
            if (frm.checkBox1.Checked && Game.IsInGame)
            {
                ...
                try
                {
                    ...
                    s = Pointer("dota2", OffsetPlayer, new int[] { 0, 0x0, 0x48, 590}, true, 1).Adress.ToString("X");
        ...
        public partial class Form1 : Form
        {
                ...
                if (frm.textBox666.Text.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            s = Pointer("dota2", OffsetPlayer, new int[] { 0, 0x0, 0x48, 590, 0x7F4 }, true, 1).Adress.ToString("X");
                ...
                if (frm.textBoxStr.Text.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            s = Pointer("dota2", OffsetPlayer, new int[] { 0, 0x0, 0x48, 590, 0x10e8 }, true, 1).Adress.ToString("X");
                            Win32.WriteProcessMemory(P[0].Handle, long.Parse(s, NumberStyles.HexNumber), buffer, buffer.Length, ref bytesWritten);
                        }
                        catch { }
                    }
                ...
                if (frm.textBoxAlg.Text.Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            s = Pointer("dota2", OffsetPlayer, new int[] { 0, 0x0, 0x48, 590, 0x10ec }, true, 1).Adress.ToString("X");
                            Win32.WriteProcessMemory(P[0].Handle, long.Parse(s, NumberStyles.HexNumber), buffer, buffer.Length, ref bytesWritten);
                        }
                        catch { }
                    }
            ...
            private void buttonInt_Click(object sender, EventArgs e)
            {
                if (frm.textBoxInt.Text.Length > 0)
                {
                    ...
                    var patern = new byte[] { 0xF3, 0x0F, 0x11, 0xBB, 0xF0, 0x10, 0x00, 0x00, 0xF3, 0x0F, 0x10, 0x8B, 0xF0, 0x10, 0x00, 0x00 };
                    long offsetmodule = 0x01C508B8;
                    //MakeCave(valueon, valueoff, patern, offsetmodule);
                    if (isNOPInt)
                    {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            s = Pointer("dota2", OffsetPlayer, new int[] { 0, 0x0, 0x48, 590, 0x10f0 }, true, 1).Adress.ToString("X");
                            Win32.WriteProcessMemory(P[0].Handle, long.Parse(s, NumberStyles.HexNumber), buffer, buffer.Length, ref bytesWritten);
                        }
                        catch { }
                    }
                ...
                if (args.Msg == 0x0101)
                {
					...
					else if (args.WParam == SubMenu.Item("RKey").GetValue<Ensage.Common.Menu.KeyBind>().Key)
                    {
                        ...
                        if(SubMenu.Item("RH").GetValue<bool>())
                        {
                        	try
                        	{
                        	 frm.label666.Text = Pointer("dota2", OffsetPlayer, new int[] { 0, 0x0, 0x48, 590, 0x128 }, false, 99999).Value.ToString("X");
				}
                        	catch { }
                        }
                        ...
                        if(SubMenu.Item("RM").GetValue<bool>())
                        {
                        	try
                        	{
	                            ...
	                            s = Pointer("dota2", OffsetPlayer, new int[] { 0, 0x0, 0x48, 590, 0x7e0 }, true, 1).Adress.ToString("X");
	                            Win32.WriteProcessMemory(P[0].Handle, long.Parse(s, NumberStyles.HexNumber), buffer, buffer.Length, ref bytesWritten);
                        	}
                        	catch { }
                        }
